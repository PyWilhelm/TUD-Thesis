\documentclass[article,type=msc,colorback,accentcolor=tud9c, 11pt]{tudthesis}
\usepackage{setspace}
\usepackage{multirow}
\usepackage[normalem]{ulem}
\useunder{\uline}{\ul}{}
\usepackage{fancyhdr}
\usepackage{amsmath}
\usepackage{hyperref}
\usepackage{float}
\usepackage{longtable}
\usepackage{enumerate}
\usepackage{enumitem}
\usepackage[figuresright]{rotating}
\usepackage{adjustbox}
\usepackage{array}
\usepackage{color}
\usepackage{caption}
\usepackage{tabularx}
\usepackage[round]{natbib}

\newcolumntype{R}[2]{%
    >{\adjustbox{angle=#1,lap=\width-(#2)}\bgroup}%
    l%
    <{\egroup}%
}

\renewcommand{\arraystretch}{1.5}
\captionsetup[figure]{font={stretch=1.3}}
\captionsetup[table]{font={stretch=1.3}}
\setlist[description]{leftmargin=1.5cm,labelindent=0.5cm}
\newcommand*\rot{\multicolumn{1}{R{45}{1em}}}
\hypersetup{
    colorlinks,
    citecolor=black,
    filecolor=black,
    linkcolor=black,
    urlcolor=black
}
\numberwithin{equation}{section}
\numberwithin{figure}{section}
\numberwithin{table}{section}
\setstretch{1.3}


\newcommand{\getmydate}{%
  \ifcase\month%
    \or Januar\or Februar\or M\"arz%
    \or April\or Mai\or Juni\or Juli%
    \or August\or September\or Oktober%
    \or November\or Dezember%
  \fi\ \number\year%
}

\renewcommand*\contentsname{Table of Content}

\begin{document}
\thesistitle{Related Articles Discovery in Large Corpora}%
{}
\author{Ziyang Li}
\birthplace{Tianjin, China}
\referee{Prof. Dr. Iryna Gurevych}{Nils Reimers}
\department{Fachbereich Informatik}
\group{Ubiquitous Knowledge Processing}
\dateofexam{\today}{\today}
\makethesistitle
\affidavit{Ziyang Li}
\setcounter{page}{1}
\input{content/0abstract}
\clearpage  
%\pagenumbering{roman}
%\setcounter{page}{1}
\tableofcontents
\input{content/0newcommand}
\clearpage

%\pagenumbering{arabic}
%\setcounter{page}{1}
\input{content/1Introduction}
\clearpage
\input{content/2State-of-art}
\clearpage
\input{content/3Task-evaluation}
\clearpage
\input{content/4Experimental-setup}
\clearpage
\input{content/5Experimental-results}
\clearpage
\input{content/6Combination-of-approaches}
\clearpage
\input{content/7Conclusion}
\clearpage
\medskip
\addcontentsline{toc}{section}{\listfigurename}
\listoffigures

\medskip
\addcontentsline{toc}{section}{\listtablename}
\listoftables
\clearpage
\medskip
\addcontentsline{toc}{section}{Bibliography}

\bibliographystyle{apalike}
\bibliography{cites}
\end{document}
